import { ticketSoporte } from '../index';


const chalk = require('chalk');

export const SOPORTE = async (client, serverInfo, colores, reaction, user) => {
    try {

        await reaction.users.remove(user)

        if (ticketSoporte.has(user.id) || reaction.message.guild.channels.cache.find(ch => ticketSoporte.has(ch.id))) {
            return user.send({
                embed: {
        ...
        await reaction.message.guild.channels.create(`Soporte-${ticketUser.username}`, {
            type: 'text',
            parent: serverInfo.testGuild.ticketParents.soporte,
            ...
            ticketSoporte.set(user.id, ch.id);
            channel = ch;
        }).catch(e => console.log(`<> ${chalk.red.bold.underline('Error')}: ${e}`))        

        const aauLogEmbed = {
            title: "️️️️️️️📫 Soporte al Usuario 📫",
        ...
        await channel.send(`<@${user.id}>`, {
            embed: aauChannelEmbed
                ...
                collector.on('collect', async (reaction, user) => {
                    ticketCollection.delete(ticketUser.id);
                await message.channel.updateOverwrite(ticketUser.id, {
                    deny: ["VIEW_CHANNEL"]
                });
                ...
                await message.channel.setName(`closed-${ticketUser.username}`)
