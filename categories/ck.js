...
export const CK = async (client, serverInfo, colores, reaction, user) => {
    try {
        ...
        if (ticketKills.has(user.id) || reaction.message.guild.channels.cache.find(ch => ticketKills.has(ch.id))) {
            return user.send({
                embed: {
        ...
        await reaction.message.guild.channels.create(`CK-${user.username}`, {
            type: 'text',
            parent: serverInfo.testGuild.ticketParents.ck,
        ...
        await channel.send(`<@${user.id}>`, {
            embed: aauChannelEmbed
                ...
                collector.on('collect', async (reaction, user) => {
                        await reaction.users.remove(user);
                        ticketKills.delete(ticketUser.id);
